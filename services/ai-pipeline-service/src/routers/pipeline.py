...
async def call_chat(text: str, user_id: str | None):
    async with httpx.AsyncClient(base_url=settings.CHAT_URL, timeout=settings.TIMEOUT) as client:
        resp = await client.post("/chat/reply", json={"text": text, "user_id": user_id})
    resp.raise_for_status()
...
async def transcribe_and_respond(audio: UploadFile = File(...), user_id: str | None = None, voice: str | None = None):
    ...
    try:
        ...
        if intent == "chit_chat":
            reply = await call_chat(text, user_id)
        else:
...
async def process_text(req: TextRequest):
    ...
    if intent == "chit_chat":
        reply = await call_chat(req.text, req.user_id)
    else:
        reply = f"Intent '{intent}' acknowledged (CRUD call omitted in demo)."
